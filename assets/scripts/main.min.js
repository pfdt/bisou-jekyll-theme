...
allowEmpty=false,disableBodyScroll=bodyScrollLock.disableBodyScroll,enableBodyScroll=bodyScrollLock.enableBodyScroll,jsonFeedUrl='/assets/feeds/recipes.json',firebase_apiKey={{site.firebase_apiKey|jsonify}},firebase_projectId={{site.firebase_projectId|jsonify}},firebase_databaseName={{site.firebase_databaseName|jsonify}},firebase_authDomain=firebase_projectId+'.firebaseapp.com',firebase_databaseURL=firebase_databaseName+'.firebaseio.com',firebase_storageBucket=firebase_projectId+'.appspot.com',signedIn_modalTitle={{site.data.taxonomies.account.signed-in_title|jsonify}},signedIn_modalBody={{site.data.taxonomies.account.signed-in_body|markdownify|jsonify}},signedIn_modalClose={{site.data.taxonomies.account.signed-in_close|jsonify}},login_modalTitle={{site.data.taxonomies.account.login_title|jsonify}},login_modalBody={{site.data.taxonomies.account.login_body|markdownify|jsonify}},login_modalClose={{site.data.taxonomies.account.login_close|jsonify}};$(document).ready(function(){var
$1em=parseFloat(getComputedStyle(document.body,null).fontSize);const
$htmlBody=$("html, body"),scrollTopOffset=$1em*6.3,wideLayoutBreakingpoint=$1em*35.5,$searchInput=$("#searchbar__input"),$emptySearch=$("#searchbar__empty-search"),$allMenuItems=$(".filter-button"),$gridContainer=$("#isotopeContent"),$bookmarksFilter=$("#filter-button--bookmarks"),$recipesAll=$("#filter-button--all-recipes"),$sidebar=document.querySelector('#sidebar'),$accountModal=document.querySelector('#account-modal');initiateLazyLoad();var $grid=$($gridContainer).isotope({itemSelector:'.grid-item',getSortData:{name:"[data-name]",date:"[data-date]",rating:"[data-rating]",},percentPosition:true,layoutMode:'fitRows',transitionDuration:0});var $items=$grid.find(".grid-item");$grid.addClass("showOn").isotope("revealItemElements",$items);document.addEventListener('ResizeObserverCompleted',function(){isotopeLayout();});detectHash();initiateFirebase();function initiateFirebase(){var config={apiKey:firebase_apiKey,authDomain:firebase_authDomain,databaseURL:firebase_databaseURL,storageBucket:firebase_storageBucket};firebase.initializeApp(config);}
firebase.auth().onAuthStateChanged(function(user){loadFirebaseDatas(user);});getThemePreference();async function initiateLazyLoad(){var lazyLoadInstance=new LazyLoad({elements_selector:".lazy",});}
...
async function signedIn(){$('body').addClass("signed-in");$('.main-nav__icons--account').css('background-image',"url('"+userPhotoURL+"')");$('.main-nav__icons--account > path').css('display','none');$('.modal-title').html(signedIn_modalTitle);$('.modal-content__body').html(signedIn_modalBody);$('.modal-content__button .modal-off').html(signedIn_modalClose);}
...
async function isotopeFilter(filterValue,noScrollTop){$grid.isotope({filter:filterValue});if(!noScrollTop){$htmlBody.animate({scrollTop:0});}}
...
currentBookmarks=filterValue;$bookmarksFilter.attr("data-filter",filterValue);if(currentFilter=="cat=bookmarks"){isotopeFilter(filterValue,true);}});}else{modalON();}}
...
function getSearchResults(){var searchValues=[];var resultsCount=0;q=q.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[\u2019]/g,"'");$.each(jsonFeed,function(index,item){let title=item.title||"";let ingredients=item.ingredients||"";let technique=item.technique||"";let category=item.category||"";let source=item.source||"";if(title.indexOf(q)>-1||ingredients.indexOf(q)>-1||technique.indexOf(q)>-1||category.indexOf(q)>-1||source.indexOf(q)>-1){searchValue=item.data_search;resultsCount++;searchValues.push(searchValue);}});if(resultsCount>0){var filterValue=searchValues.join(", ");currentFilter="q="+encodeURIComponent(q);$("#no-result").removeClass("active");pushURL();}else{var filterValue=":not(*)";$("#no-result").addClass("active");}
layoutCleanFilter($emptySearch);isotopeFilter(filterValue);}
async function preloadImages(){var img=["{{ '/assets/img-static/alt_logo-icon.svg' | relative_url }}","{{ '/assets/img-static/alt_star--borders.svg' | relative_url }}","{{ '/assets/img-static/alt_star.svg' | relative_url }}","{{ '/assets/img-static/alt_bookmark.svg' | relative_url }}","{{ '/assets/img-static/alt_arrow--up.svg' | relative_url }}","{{ '/assets/img-static/alt_arrow--down.svg' | relative_url }}","{{ '/assets/img-static/alt_logo.svg' | relative_url }}","{{ '/assets/img-static/alt_bookmarks.svg' | relative_url }}","{{ '/assets/img-static/alt_star--dots.svg' | relative_url }}","{{ '/assets/img-static/alt_bookmark--border.svg' | relative_url }}","{{ '/assets/img-static/alt_sort--name.svg' | relative_url }}","{{ '/assets/img-static/alt_sort--date.svg' | relative_url }}","{{ '/assets/img-static/alt_sort--rating.svg' | relative_url }}","{{ '/assets/img-static/logo-icon.svg' | relative_url }}","{{ '/assets/img-static/star--borders.svg' | relative_url }}","{{ '/assets/img-static/star.svg' | relative_url }}","{{ '/assets/img-static/bookmark.svg' | relative_url }}","{{ '/assets/img-static/arrow--up.svg' | relative_url }}","{{ '/assets/img-static/arrow--down.svg' | relative_url }}","{{ '/assets/img-static/logo.svg' | relative_url }}","{{ '/assets/img-static/bookmarks.svg' | relative_url }}","{{ '/assets/img-static/star--dots.svg' | relative_url }}","{{ '/assets/img-static/bookmark--border.svg' | relative_url }}","{{ '/assets/img-static/sort--name.svg' | relative_url }}","{{ '/assets/img-static/sort--date.svg' | relative_url }}","{{ '/assets/img-static/sort--rating.svg' | relative_url }}"];for(let i=0;i<img.length;i++){$("head").append("<link rel='preload' href='"+img[i]+"' as='image'>");}}});