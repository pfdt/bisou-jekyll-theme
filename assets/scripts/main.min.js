...
function toggleBookmark(bookmark){if(currentUserID){var recipe=bookmark.attr("value");var path=currentUserID+"/recipes/"+recipe;const qToggleBookmark=query(ref(db,path));return onValue(qToggleBookmark,(snapshot)=>{var filterValue=currentBookmarks;if(snapshot.val()==null||snapshot.val().bookmark==undefined||snapshot.val().bookmark==false){bookmark.addClass("active");filterValue+=",.r_"+recipe;filterValue=filterValue.replace(/^,/,"").replace(",,",",");let data=snapshot.val()||{};data.bookmark=true;let updates={};updates[path]=data;update(ref(db),updates);}else{bookmark.removeClass("active");filterValue=filterValue.replace(".r_"+recipe,"").replace(",,",",").replace(/,$/,"").replace(/^,/,"");let data=snapshot.val()||{};data.bookmark=false;let updates={};updates[path]=data;update(ref(db),updates);}
...
let data=snapshot.val()||{};data.rating=stars;let updates={};updates[path]=data;update(ref(db),updates);$(".r_"+recipe).attr("data-rating",stars);isotopeSortUpdate();},{onlyOnce:true});}else{modalON();}}
